...
def index_get():
    ...
    ctx = {"posts": query_result, "current_page": page}
...
def author_index_get(username: str) -> str:
    ...
    query = (
        ...
        .filter_by(AuthorID=author.id)
        .order_by(Post.CreatedTime.desc())
    )
    query_result = db.paginate(select=query, per_page=16, page=page, error_out=True)

    ctx = {"author": author, "posts": query_result, "current_page": page}
...
def show_post_by_slug_get(slug):
    ...
    if (
        not query_result
    ):  # TODO: replace it with a custom template like blog post is not found, see similars
        abort(404)

    ctx = {"post": query_result, "author": get_admin(query_result.AuthorID)}
