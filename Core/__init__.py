...
from .extensions import db, ServerSession, ServerMigrate, ServerMail, babel, csrf
...
def create_app(setting: Setting) -> Flask:
    ...
    babel.init_app(  # babel
        ...
        default_translation_directories=str(
            (Setting.BASE_DIR / "translations").absolute()
        ),
    )
    ...
    ServerCaptcha2 = ServerCaptchaMaster.getGoogleCaptcha2(
        name="g-captcha2", conf=Setting.GOOGLE_CAPTCHA_V2_CONF
    )
    ServerCaptcha3 = ServerCaptchaMaster.getGoogleCaptcha3(
        name="g-captcha3", conf=Setting.GOOGLE_CAPTCHA_V3_CONF
    )
