...
class User(BaseModel):
    ...
    Password: so.Mapped[str] = so.mapped_column(
        sa.String(162), nullable=False, unique=True
    )
    ...
    Address: so.Mapped[str] = so.mapped_column(
        sa.String(2048), nullable=True, unique=False
    )
    PhoneNumber: so.Mapped[str] = so.mapped_column(
        sa.String(14), nullable=True, unique=True
    )
...
class StenCilOrder(BaseModel):
    ...
    Size: so.Mapped[str] = so.mapped_column(
        sa.String(256), unique=False, nullable=False
    )
    ...
    Side: so.Mapped[str] = so.mapped_column(
        sa.String(256), unique=False, nullable=False
    )
    ...
    Quantity: so.Mapped[int] = so.mapped_column(
        sa.Integer, nullable=False, unique=False
    )
    ...
    Thickness: so.Mapped[str] = so.mapped_column(
        sa.String(256), unique=False, nullable=False
    )
    ...
    Fiducials: so.Mapped[str] = so.mapped_column(
        sa.String(256), unique=False, nullable=False
    )
    ...
    Status: so.Mapped[str] = so.mapped_column(
        ...
        unique=False,
        nullable=False,
        default=list(STATUS_ENUM.keys())[2],
    )
    ...
    OtherRequest: so.Mapped[str] = so.mapped_column(
        sa.String(2048), nullable=True, unique=False
    )
    File: so.Mapped[str] = so.mapped_column(
        sa.JSON, unique=False, nullable=True, default=json.dumps([])
    )
    Address: so.Mapped[str] = so.mapped_column(
        sa.String(2048), nullable=False, unique=False
    )

    UserID: so.Mapped[int] = so.mapped_column(
        sa.Integer, sa.ForeignKey(User.id), unique=False, nullable=False
    )
...
class Ticket(BaseModel):
    ...
    Title: so.Mapped[str] = so.mapped_column(
        sa.String(64), nullable=False, unique=False
    )
    Caption: so.Mapped[str] = so.mapped_column(
    ...
    UserID: so.Mapped[int] = so.mapped_column(
        sa.Integer, sa.ForeignKey(User.id), nullable=False, unique=False
    )
...
class AnswerTicket(BaseModel):
    ...
    AdminID: so.Mapped[int] = so.mapped_column(
        sa.Integer,
        sa.ForeignKey(BaseModel.SetTableName("admins") + ".id"),
        nullable=False,
        unique=False,
    )
    TicketID: so.Mapped[int] = so.mapped_column(
        sa.Integer,
        sa.ForeignKey(BaseModel.SetTableName("tickets") + ".id"),
        nullable=True,
        unique=False,
    )
...
class NewsLetter(BaseModel):
    ...
    Email: so.Mapped[str] = so.mapped_column(
        sa.String(1024), nullable=False, unique=True
    )
    VerifiedAT: so.Mapped[str] = so.mapped_column(
        sa.DateTime, nullable=False, unique=False
    )
