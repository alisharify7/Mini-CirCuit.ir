...
class Setting:
    ...
    PRIVATE_STORAGE = BASE_DIR / "PrivateStorage"
    make_sure_directory_exists(PRIVATE_STORAGE)

    PRIVATE_STORAGE_UPLOADS = PRIVATE_STORAGE / "Uploads"
    make_sure_directory_exists(PRIVATE_STORAGE_UPLOADS)

    PCB_ATTACHMENTS = PRIVATE_STORAGE_UPLOADS / "PCB_ATTACHMENTS"
    make_sure_directory_exists(PCB_ATTACHMENTS)
    ...
    SESSION_REDIS = (
        redis.Redis.from_url(os.environ.get("REDIS_SESSION_URI"))
        if os.environ.get("REDIS_SESSION_URI", None)
        else REDIS_DEFAULT_INTERFACE
    )
    ...
    GOOGLE_CAPTCHA_V3_CONF = {
        ...
        "CAPTCHA_SCORE": (
            float(os.environ.get("CAPTCHA_SCORE_V3", 0.5))
            if os.environ.get("CAPTCHA_SCORE_V3", "0.5").isdigit()
            else os.environ.get("CAPTCHA_SCORE_V3", 0.5)
        ),
