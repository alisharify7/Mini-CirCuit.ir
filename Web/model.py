...
class NewsLetter(BaseModel):
    ...
    def is_subscribed(cls, email:str) -> bool:
        ...
        query = db.session.select(cls).filter(Email == email)
...
class ContactUs(BaseModel):
    ...
    Message: so.Mapped[str] = so.mapped_column(sa.String(4096), unique=False, nullable=False)
    is_answered: so.Mapped[bool] = so.mapped_column(sa.Boolean, default=False)
    admin_id: so.Mapped[int] = so.mapped_column(sa.Integer, sa.ForeignKey(Admin.id), nullable=True, unique=False)
