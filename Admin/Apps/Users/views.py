...
def create_new_user_post():
    ...
    if form.validate():
        ...
        if user := User.query.filter_by(
            ...
            if user.Active:  # if user wa
                ...
                if AuthUtils.get_activation_email_slug_redis(
                    ...
                    form.EmailAddress.errors.append(
                        "شما {} دقیقه دیگر میتوانید اقدام به ساخت حساب کاربری کنید".format(
                            AuthUtils.get_activation_ttl_slug_redis(
        ...
        if not u.Active:  # send activation mail
            if not (
                token := AuthUtils.gen_and_set_activation_slug(
                    email=form.EmailAddress.data
                )
            ):
            ...
            flash(
                "ایمیل تایید حساب کاربری با موفقیت برای ایمیل مورد نظر ارسال شد",
                "success",
            )
